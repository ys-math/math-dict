...
収録語数は **211 語**です。math-study の `tex/` 以下のディレクトリと 1 対 1 に対応するファイルに分かれています。
...
内訳は名詞 168 語、形容動詞 12 語、サ変名詞 7 語、数学者名 7 名（米田、グロタンディーク、ハウスドルフ、Nakayama、Morita、Yoneda、Grothendieck）、ラテン文字表記 17 語（原典で実際に使われている作用素名 `Hom` `Ker` `Im` `Ob` `Mor` `dom` `cod` `id` `GL` `Cone` `Nat` `op` `Proj` `Spec` `Map` と `iff`、`well-defined`）です。
...
`ほもとぴー` から `ホモトピー` が出るのは、この辞書の働きではありません。IME は仮名入力をそのままカタカナに変換できるので、これらの項目は IME が元から出す候補と同じ文字列を返すだけです。**9 語**がこれに当たります（イデアル、モナド、グロタンディーク、シングルトン、ソート、ハウスドルフ、ホモトピー、レトラクト、アルファステップ）。件数だけ [REVIEW.md](REVIEW.md) の「IMEが自前で変換できる項目」に記載しています。

残る 202 語は実際に働きます。恒等射・冪等行列・局所的に小さい圏 のような漢字語、`グロタンディーク群`・`コンパクト開位相` のようなカタカナと漢字の混在、`Hom`・`well-defined` のようなラテン文字表記です。
...
**語そのものは実在します。**すべて `.tex` から拾ったものなので、存在しない語が紛れ込むことはありません。**問題は読みです。**math-study の大半にはルビが無いため、読みは人の目で確認したもの（82 語、大半は以前の版から引き継いだもの）、著者が `\term{…}{…}` の定義で与えている読みをそのまま採ったもの（27 語）、生成したもの（102 語）のいずれかです。うち 18 語は `low-confidence` を付けて [REVIEW.md](REVIEW.md) に挙げてあります。特に怪しいのはラテン文字の作用素名で、`Ob`・`Mor`・`dom`・`cod`・`Proj` には定まった日本語の読みがありません。
